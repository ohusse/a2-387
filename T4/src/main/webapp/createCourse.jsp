...
<div class = "container2">
  <h1>Add Course</h1>
  <form action="<%= request.getContextPath() %>/registerCourseServlet" method="post">
    ...
    <select name='adminID' id='adminID'>
      ...
      <% while (result.next()){%>
      <%="<option value='"%><%=result.getString("adminID")%><%="'>"%>
      <%=result.getString("firstName")%>
      <%=result.getString("lastName")%><%="</option>"%>
      <%}%>
    </select><br>
    ...
    <select name="semesterID" id="semesterID">
      ...
      <% while (result2.next()){%>
      <%="<option value='"%><%=result2.getString("semesterID")%><%="'>"%>
      <%=result2.getString("season")%>
      <%=result2.getString("year")%><%="</option>"%>
      <%}%>
    </select><br>
